...
import net.ion.emotion.AffectWord;
...
public class LexicalUtility {
	...
	private String fileNameLexicon = "/net/ion/emotion/data/lex/synesketch_lexicon.txt";
	private String fileNameEmoticons = "/net/ion/emotion/data/lex/synesketch_lexicon_emoticons.txt";
	private String fileNameProperties = "/net/ion/emotion/data/lex/keywords.xml";
	...
	/**
	 * Returns true if the word and the negation are in the same part of the sentence, i.e. divided by a interpunction mark.
	 * 
	 * @param word
	 *            {@link String} which represents a word
	 * @return boolean, true is the word is an intensity modifier
	...
	public static boolean inTheSamePartOfTheSentence(String negation, String word, String sentence) {
		...
		for (int k = i; k < j; k++) {
			if ((sentence.charAt(k) == ',') || (sentence.charAt(k) == '.') || (sentence.charAt(k) == ';') || (sentence.charAt(k) == ':') ||
			// (sentence.charAt(k) == '\'') ||
					(sentence.charAt(k) == '-')) {
				return false;
			}
