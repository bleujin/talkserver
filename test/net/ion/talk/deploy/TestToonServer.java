...
public class TestToonServer extends TestCase {
	...
	public void testRun() throws Exception {
		...
		Configuration config = ConfigurationBuilder.newBuilder().aradon().addAttribute(RepositoryEntry.EntryName, repository).addAttribute(RhinoEntry.EntryName, rengine).sections().restSection("script").path("jscript").addUrlPattern("/{name}.{format}").matchMode(IMatchMode.STARTWITH)
				.handler(ScriptExecLet.class).restSection("auth").addPreFilter(new ChallengeAuthenticator("users", verifier)).path("login").addUrlPattern("/login").matchMode(IMatchMode.STARTWITH).handler(LoginLet.class).build();
		...
		Radon radon = RadonConfiguration.newBuilder(9000).add("/script/*", ahandler).add("/auth/*", ahandler).add("/websocket/{id}", tengine).add("/resource/*", new ResourceFileHandler("./resource/")).createRadon();
